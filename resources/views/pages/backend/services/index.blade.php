<x-admin-layout>
    <a href="{{ route('admin.services.create') }}" class="container flex justify-center">
        <x-backend.submit-button>
            {{ __('Add') }}
        </x-backend.submit-button>
    </a>
    <div class="container flex justify-center mx-auto w-screen">
        <div class="flex flex-col">
            <div class="w-full">
                <div class="border-b border-gray-200 shadow">
                    <table>
                        <thead class="bg-gray-50 w-screen ">
                            <tr>
                                <th class="px-6 py-2 text-xs text-gray-500 ">
                                    ID
                                </th>
                                <th class="px-6 py-2 text-xs text-gray-500">
                                    Title </th>
                                <th class="px-6 py-2 text-xs text-gray-500 overflow:hidden">
                                    Description
                                </th>
                                <th class="px-6 py-2 text-xs text-gray-500">
                                    Image </th>
                                <th class="px-6 py-2 text-xs text-gray-500">
                                    View
                                </th>
                                <th class="px-6 py-2 text-xs text-gray-500">
                                    Edit
                                </th>
                                <th class="px-6 py-2 text-xs text-gray-500">
                                    Delete
                                </th>
                            </tr>
                        </thead>
                        <tbody class="bg-white">
                            @foreach ($item as $items)
                            <tr class="whitespace-nowrap">
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    {{ $items->id }}
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900 overflow-hidden">
                                        {{ $items->title }}
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-500 overflow-hidden">
                                        <p>{{ $items->description }}</p>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    <img src="{{ asset('Upload/images/servicesp/'. $items->image) }}" width="70px" height="100px" alt="">
                                </td>
                                <td class="px-6 py-4">
                                    <a href="{{ route('admin.services.view', $items->id) }}" class="px-4 py-1 text-sm text-white  rounded">
                                        <img src="data:image/png;base64,">
                                    </a>
                                </td>
                                <td class="px-6 py-4">
                                    <a href="{{ route('admin.services.edit', $items->id) }}" class="px-4 py-1 text-sm text-white  rounded">
                                        <img src="data:image/png;base64,">
                                    </a>
                                </td>
                                <td class="px-6 py-4">
                                    <a href="{{ route('admin.services.delete', $items->id) }}" class="px-4 py-1 text-sm text-white  rounded" onclick="return confirm('are u sure do u want to delete this user?');">
                                        <span x-on:click="open = true">
                                            <img src="data:image/png;base64,">
                                    </a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
    </x-app-layout>