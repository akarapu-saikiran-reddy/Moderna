<x-admin-layout>
    <x-backend.card class="">
        ...
        <form action="{{ route('admin.portfolio.update', $portfolio->id) }}" method="post" enctype="multipart/form-data">
            ...
            <x-backend.form.input type="file" placeholder="App" label="app" name="app" value="{{ $portfolio->app }}" />
            <img src="{{ asset('Upload/images/portfolio/web/' . $portfolio->web) }}" height="150px" width="150px" alt="">
            <x-backend.form.input type="file" placeholder="Web" label="web" name="web" value="{{ $portfolio->web }}" />
            <img src="{{ asset('Upload/images/portfolio/card/' . $portfolio->card) }}" height="150px" width="150px" alt="">
            <x-backend.form.input type="file" placeholder="Descrioption" label="Description" name="description" value="{{ $portfolio->card }}" />
