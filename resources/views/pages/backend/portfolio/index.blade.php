<x-admin-layout>
    <x-backend.card>
        <x-slot name="title">
        </x-slot>
        <x-slot name="addbutton">
            <a href="{{ route('admin.portfolio.create') }}">
                <x-backend.hit-button>
                    {{ __('Add ') }}
                </x-backend.hit-button>
            </a>
        </x-slot>
        <div class="container flex justify-center mx-auto w-full ">
            <div class="flex flex-col">
                <div class="w-full">
                    <div class="border border-gray-900 shadow">
                        <table>
                            <thead class=" w-full rounded-lg shadow-lg justify-center">
                                <tr>
                                    <th class="px-6 py-2 text-xs  text-gray-200 text-center" width="80">
                                        ID
                                    </th>
                                    <th class="px-6 py-2 text-xs text-gray-200" width="150">
                                        App </th>
                                    <th class="px-6 py-2 text-xs text-gray-200 overflow:hidden" width="200">
                                        Web
                                    </th>
                                    <th class="px-6 py-2 text-xs text-gray-200" width="200">
                                        Card </th>
                                    <th class="px-6 py-2 text-xs text-gray-200" width="200">
                                        Action
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($item as $items)
                                <tr class="whitespace-nowrap">
                                    <td class="px-6  text-sm text-gray-900">
                                        {{ $items->id }}
                                    </td>
                                    <td class="px-6  text-sm text-gray-900">
                                        <div class="rounded-full overflow-hidden">
                                            <img src="{{ asset('Upload/images/portfolio/app/'.$items->app) }}" width="70px" height="100px" alt="">
                                        </div>
                                    </td>
                                    <td class="px-6  text-sm text-gray-900">
                                        <div class="rounded-full overflow-hidden">
                                            <img src="{{ asset('Upload/images/portfolio/web/'.$items->web) }}" width="70px" height="100px" alt="">
                                        </div>
                                    </td>
                                    <td class="px-6  text-sm text-gray-900">
                                        <div class="rounded-full overflow-hidden">
                                            <img src="{{ asset('Upload/images/portfolio/card/'.$items->card) }}" width="70px" height="100px" alt="">
                                        </div>
                                    </td>
                                    <td class="px-6 ">
                                        <div class="row">
                                            <div class="col-4">
                                                <a href="{{ route('admin.team.view', $items->id) }}" class="px-4 py-1 text-sm text-white  rounded">
                                                    <img src="data:image/png;base64,">
                                                </a>
                                            </div>
                                            <div class="col-4">
                                                <a href="{{ route('admin.team.edit', $items->id) }}" class="px-4 py-1 text-sm text-white  rounded">
                                                    <img src="data:image/png;base64,">
                                                </a>
                                            </div>
                                            <div class="col-4">
                                                <a href="{{ route('admin.team.delete', $items->id) }}" class="px-4 py-1 text-sm text-white  rounded" onclick="return confirm('are u sure do u want to delete this user?');">
                                                    <img src="data:image/png;base64,">
                                                </a>
                                            </div>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </x-backend.card>
</x-admin-layout>