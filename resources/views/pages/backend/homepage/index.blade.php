<x-admin-layout>
    <a href="{{ route('admin.homepage.create') }}" class="container flex justify-center">
        <x-backend.submit-button>
            {{ __('Add') }}
        </x-backend.submit-button>
    </a>
    <div class="container flex justify-center mx-auto w-screen">
        <div class="flex flex-col">
            <div class="w-full">
                <div class="border-b border-gray-200 shadow">
                    <table>
                        <thead class="bg-blue-600 w-full">
                            <tr>
                                <th class="px-6 py-2 text-xs text-gray-500 ">
                                    ID
                                </th>
                                <th class="px-6 py-2 text-xs text-gray-500">
                                    Title </th>
                                <th class="px-6 py-2 text-xs text-gray-500 overflow:hidden">
                                    Description
                                </th>
                                <th class="px-6 py-2 text-xs text-gray-500">
                                    Image </th>
                                <th class="px-6 py-2 text-xs text-gray-500">
                                    Action
                                </th>
                            </tr>
                        </thead>
                        <tbody class="bg-white">
                            @foreach ($item as $items)
                            <tr class="whitespace-nowrap">
                                <td class="px-6  text-sm text-gray-500">
                                    {{ $items->id }}
                                </td>
                                <td class="px-6 ">
                                    <div class="text-sm text-gray-900 overflow-hidden">
                                        {{ $items->title }}
                                    </div>
                                </td>
                                <td class="px-6 ">
                                    <div class="text-sm text-gray-500 overflow-hidden">
                                        <p>{{ $items->description }}</p>
                                    </div>
                                </td>
                                <td class="px-12  text-sm text-gray-500">
                                    <img src="{{ asset('Upload/images/homepage/' . $items->image) }}" width="120px" class="rounded-circle">
                                </td>
                                <td class="px-6 ">

                                    <div class="row">
                                        <div class="col-4">
                                            <a href="{{ route('admin.homepage.view', $items->id) }}" class="px-4 py-1 text-sm text-white  rounded">
                                                <img src="data:image/png;base64,">
                                            </a>
                                        </div>
                                        <div class="col-4"> <a href="{{ route('admin.homepage.edit', $items->id) }}" class="px-4 py-1 text-sm text-white  rounded">
                                                <img src="data:image/png;base64,">
                                            </a></div>
                                        <div class="col-4"> <a href="{{ route('admin.homepage.delete', $items->id) }}" class="px-4 py-1 text-sm text-white  rounded" onclick="return confirm('are u sure do u want to delete this user?');">
                                                <span x-on:click="open = true">
                                                    <img src="data:image/png;base64,">
                                            </a></div>
                                    </div>



                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</x-admin-layout>